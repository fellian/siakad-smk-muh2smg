...
@section('content')
    <div class="bg-white rounded-lg shadow">
        <div class="p-6 border-b">
            ...
            <p class="text-sm text-gray-500 mt-1">Tahun Ajaran: {{ $tahunAjaran?->tahun ?? '-' }} - Semester
                {{ $tahunAjaran?->semester ?? '-' }}</p>
        </div>
        <div class="p-6">
                    ...
                    @foreach($kelasDiampu as $jadwal)
                        <div class="border rounded-lg p-4 hover:shadow-md transition-shadow">
                            <div class="flex items-center justify-between mb-3">
                                <span
                                    class="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs">{{ $jadwal->kelas->nama_kelas }}</span>
                            </div>
                            ...
                            <div class="flex gap-2">
                                <a href="{{ route('guru.nilai.input', ['kelas_id' => $jadwal->kelas_id, 'mapel_id' => $jadwal->mata_pelajaran_id]) }}"
                                ...
                                <a href="{{ route('guru.nilai.rekap', ['kelas_id' => $jadwal->kelas_id, 'mapel_id' => $jadwal->mata_pelajaran_id]) }}"
