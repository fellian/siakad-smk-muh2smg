...
<div class="max-w-2xl mx-auto bg-white rounded-lg shadow">
    <div class="p-6 border-b">
        <h3 class="text-lg font-semibold">Form Tambah Mata Pelajaran</h3>
    </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="text" name="kode_mapel" value="{{ old('kode_mapel') }}" 
                       class="w-full border rounded-lg px-4 py-2 @error('kode_mapel') border-red-500 @enderror" 
                       placeholder="Contoh: MTK10" maxlength="20">
                @error('kode_mapel')
            ...
            <div>
                ...
                <input type="number" name="kkm" value="{{ old('kkm', 75) }}" 
                       class="w-full border rounded-lg px-4 py-2 @error('kkm') border-red-500 @enderror" 
                       min="0" max="100">
                @error('kkm')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
