...
<head>
    ...
    <title>@yield('title', 'Guru Dashboard') | SIAKAD SMK Muhammadiyah 2</title>
                        ...
                        <div class="relative">
                            <button @click="profileOpen = !profileOpen" @click.away="profileOpen = false" class="flex items-center space-x-3 focus:outline-none hover:bg-gray-50 p-1.5 rounded-xl transition-colors">
                                <div class="hidden md:block text-right">
                                    <p class="text-sm font-bold text-blue-900 leading-tight">{{ auth()->user()->name ?? 'Guru' }}</p>
                                    <p class="text-[11px] font-medium text-gray-500">Guru</p>
                                </div>
                                <div class="w-9 h-9 rounded-full bg-blue-50 border-2 border-white shadow-sm flex items-center justify-center overflow-hidden shrink-0">
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name ?? 'Guru') }}&background=EFF6FF&color=1D4ED8&bold=true" alt="Profile" class="w-full h-full object-cover">
                                </div>
                            </button>
                            ...
                            <div x-show="profileOpen" 
                                ...
                                <a href="{{ route('guru.profile.index') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors">
